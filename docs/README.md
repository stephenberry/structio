...
| | |
|---|---|
| [Schemas and types](schemas.md) | Declaring a struct's schema as an object or as an array, renaming keys one at a time or by a case rule, marking members the document has to carry, generics and borrowing, the supported type set, and writing the impls by hand. |
| [The derive](derive.md) | `#[derive(Structio)]`, behind the `derive` feature: each attribute and the macro syntax it expands to, generics read off the type, what is refused and why, the later stages, and a table for anyone coming from serde. |
| [Enums](enums.md) | Declaring a unit or tagged enum, the two wire forms and which of them reading accepts, renaming variants one at a time or by a case rule, generics and borrowing, what is refused and with which error, how the policies meet a tag, and the string array a run of unit variants becomes in BEVE. |
| [BEVE](beve.md) | What the binary format buys you, how it differs from JSON in practice, reaching one field without decoding the rest, turning a document you have no type for into JSON, complex numbers and matrices, writing arrays a reader can point at and borrowing one back, framing a body whose length has to be sent before it or that lands behind a header, reaching the typed-array path from a type this crate does not describe, and what is not implemented yet. |
