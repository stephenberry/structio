...
**A delimiter is never a value.** The specification's delimiter extension separates documents in a stream, where `beve::Documents::values` steps over it. Anywhere a value belongs, including as a whole document, every walk that takes whatever value is there refuses it as `InvalidHeader`: validating, skipping an unknown member, reading a `Value`, transcoding, and framing a stream alike. A typed read that wanted a particular kind says so instead, `ExpectedNumber` and the like, as it does for any header that is not what it wanted. It is malformed rather than `UnsupportedFeature`, since it is not a construct some other decoder could read as a value, and a validator that stepped over it as a value of no extent would pass a document that frames as nothing and that no reader can read.
...
- **The deprecated type tag is refused**, with `UnsupportedFeature`. It names a variant by an index whose meaning is not in the document, so it has no JSON form to take. The other two extensions that are values are written: a complex number becomes `[re,im]` and a matrix becomes `{"layout":…,"extents":[…],"value":[…]}`, which are not encodings chosen here: they are what `Complex` and `Matrix` write in JSON and read back from BEVE, so a document that goes through a transcode still loads into the same types. A delimiter is `InvalidHeader`, as it is everywhere a value belongs.
...
The same document, laid out differently. Every reader here takes either form, so nothing on the way back has to know which one it was given, and what it costs is the padding plus a form a decoder is less likely to have implemented. Only numeric arrays wider than one byte change: booleans and strings have no aligned form, and one-byte elements are aligned wherever they land. A run of complex numbers counts, its components being numbers, and takes the complex extension's own aligned form: the class header, then an aligned typed array of the interleaved components, so a `&[Complex<f64>]` can be borrowed the same way. A document with no numeric array wider than a byte in it comes out byte for byte the same as `to_beve` would have written it. A matrix gets it for free, since its data is an ordinary typed array or a complex run.
...
| `to_beve_aligned(&T) -> Vec<u8>` | Serialize with numeric and complex arrays in the zero-copy aligned form. |
