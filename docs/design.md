...
- **Missing keys are silent by default.** A field absent from the document keeps whatever the value already held, which for `from_str` is its `Default`. Marking it [`#[required]`](schemas.md#required-fields) makes that field's absence a `MissingKey` under every policy; [`RequireKeys`](options.md#error_on_missing_keys) does the same for every field at once.
...
Glaze returns an error from writing, and safety-critical work is the case where that matters, since a system that must not abort has to serialize into memory it owns and be told when the value does not fit. So the question is whether `Write::write` should return a `Result`. It should not, and the reason is that it would not deliver the thing it appears to.

**Allocation failure is not reachable through the return type.** Every append funnels through `room`, and the only allocation on the write path is the `buf.reserve(n)` in the `#[cold]` `spill` beneath it. `Vec::reserve` calls `handle_alloc_error`, which aborts; it does not return, so there is no point at which an `Err` could be constructed and passed back out. Putting `?` on every impl and every macro-generated body would change nothing about what happens when the allocator says no. serde is the worked example: `Serialize::serialize` returns a `Result` at every level and `serde_json::to_vec` still aborts on OOM, because that `Result` is for io and for data the format cannot represent. The lever is the allocation policy rather than the signature, and it is one call: `Vec::try_reserve` is stable well inside the MSRV.

**The channel Glaze uses is the one already here.** Glaze does not return an error from each value's write either; it threads a context, an implementation sets `ctx.error`, and the entry point returns it. `Writer` has exactly that for io: `Sink::err` records the first failure, everything after it is discarded so a broken sink cannot grow the buffer without bound, and `finish` surfaces it once. Reporting a write failure means generalizing that field from io to the writer, not changing what a `Write` impl looks like.

**What a certified system needs is not a recoverable OOM.** DO-178C and ISO 26262 practice is to allocate at initialization and never grow, so an out-of-memory branch in the message path is unreachable by construction, which also makes it untestable, which is worse than not having it. The requirement is a bounded destination and a deterministic answer to whether the value fits. For BEVE that path is already complete: `beve::size_after(&value, out.len())` measures exactly, a debug build checks the measurement against what the write produces, and appending into a buffer that already has the capacity allocates nothing at all. For JSON there is no `size`, and that is not neglect: an exact JSON length means running the float formatting, so measuring costs about what writing costs.

**The hole is the adapter.** `WriteAs::write` tells an implementation whose target has values it cannot encode to write a documented substitute or panic. Under `panic = "abort"`, which is what a system that forbids unwinding compiles with and what this repository's own release profile sets, that rule means abort the process, and there is no third answer. `json::append` and `beve::append` restore the caller's buffer when a write unwinds, which serves a build that unwinds and no other.

**The shape if it gets built.** Boundedness belongs on `Options`, for the reason measuring does: the policy parameter is the only thing every `Write::write` signature already carries. A `Bounded<O>` beside `Measured<O>` would let `room` refuse instead of grow, with the refusal folded away per instantiation, so the ordinary writer keeps the single compare it has now. Then an error field beside `Sink::err`, a `Writer::fail` an adapter can call instead of panicking, and entry points that return a `Result` and never grow the buffer past the capacity it arrived with. Leaving that buffer as it was found needs nothing new: it is the guard the append entry points already carry to survive an unwind, with the truncation never lifted. `try_reserve` in `spill` is a few lines once there is somewhere to report it, and turns the growable case from an abort into an error.

Three costs, none fatal and none zero. A bounded write monomorphizes a second copy of every `Write` impl, so a caller who uses both modes pays code size and one who uses only the bounded mode pays nothing. `Measured<O>` gains a sibling that forwards `Options` by hand, which is the list this file already admits is guarded by a note rather than by a test. And unlike a change that removes work, this one trades a folded-away branch for a growth path, so the claim that the ordinary writer is unaffected has to be measured rather than asserted.
...
The same three hooks exist on the adapter traits, and they have to. An adapter is how a type from another crate is described at all, and a foreign *scalar* is the case where the block matters most, so a mechanism that could not express one would have closed the format's headline feature off from the escape hatch the orphan rule makes necessary. `write_slice_with` therefore dispatches on `WriteAs::ARRAY` rather than `Write::ARRAY`, and `try_bulk_with` offers the payload to `ReadAs::read_bulk` rather than to `Read::read_bulk`; `Same` forwards both, which is what makes it an identity on the bytes and on the work rather than only on the values. All three default to the generic form, so an adapter with a conversion to do -- which is most of them, and has no block to copy -- says nothing and gets what it always got, minus the array header `try_bulk_with` reads and puts back. That probe is the one the unadapted path has always made, and identically enough that the two fold to a single function in the object file.

What an adapter cannot borrow from the crate is the claim that a type's memory *is* a payload. That claim is `NumericBytes`, which is `unsafe` and, for the same reason, not sealed: the crate's own impls cover the numbers BEVE can name, and the one case they cannot reach is a scalar somebody else declared. Three of its four clauses are the author's; the fourth, that an element is the width its declared header names, is a constant of a generic type and so a build error rather than a silent misparse. `Reader::read_block` and `Writer::write_block` are the copy itself, and neither is `unsafe`, because every bit pattern of such a type is a value and the worst a wrong call yields is a cursor left inside the next value. That is a bad answer, not an unsound one, and it is the same failure mode the crate's own bulk impls guard against with the element-header test at the top of `read_bulk`.
...
Two consequences fall out of that rather than being arranged. A document the validator accepts transcodes, unless it holds a value JSON has no form for at all -- a 128-bit float, or the deprecated type tag -- or a matrix layout byte outside the two that are defined, which is asserted as a property rather than argued. And the output is not merely valid JSON but the *same* JSON the typed writer produces. That one does not follow from sharing anything -- the typed path goes through `Write` impls and `ToJsonKey` while the transcoder hand-rolls the writer's primitives -- which is why it is asserted rather than assumed.
...
The second is depth. A complex array is a sequence, and every other sequence charges a level, but this one charges none -- in `read_seq`, in the bulk copy, in `skip_value`, in the transcoder, and in the splitter. It holds numbers and nothing else, so no walk over one ever recurses, and what actually matters is that they all agree: charging it in one place and not another is how a validator comes to pass, at the very last level, a document the reader then refuses.
...
Skipping is where a reader is most easily wrong and least easily caught, because getting an extent wrong does not fail: it moves the cursor into the middle of a value and the *next* field is parsed from there. So where the specification leaves something undefined, this reader refuses rather than guesses. The complex header is the case that matters. Its form field is three bits wide, so that the class and byte count line up with a number header, but only 0, 1 and 2 are defined, and the three differ by what precedes the payload: nothing, a `SIZE`, or an aligned typed array's preamble. Guessing at 3 through 7 would make the extent of a value depend on bits that carry no meaning, so they are an `InvalidHeader`. A matrix's layout byte is refused later rather than here, and deliberately: it threatens no extent, so `validate` has no reason to look at it, while `Matrix` and the transcoder both refuse a third value with `InvalidMatrixLayout` rather than name a layout and transpose the data. Aligned typed arrays are read, skipped, and driven as numbers, all three through one shared walk of the preamble, because those three have to agree on where such an array ends. The aligned complex run goes through the complex header's walk, which consumes its inner array's preamble and reports a run like the plain one, so nothing past it knows which form it was.
...
Three functions reinterpret memory: `Reader::read_block`, `Writer::write_block`, and the borrow. What may be reinterpreted was once prose under a `# Safety` heading, held up by the discipline that only two macros ever called it; it is now the `NumericBytes` bound. That is an unsafe trait, implemented here for the fourteen fixed-width primitives and for `Complex<T>` at the twelve widths BEVE has a class header for, and implementing it asserts no padding, every bit pattern a value, that the little-endian bytes are the wire form for the element header it declares, and that an element is the width that header names. The last of those is the part a compiler can check, and does. It used to be a `const` assert beside each implementation, which only reached the implementations in this file; it is now `Block::<T>::WIDTH`, a constant of a generic type standing where `size_of::<T>()` used to stand at every site that measures a payload, so the check reaches an impl written anywhere and is load bearing rather than a dangling assertion. A payload of `n` elements is `n * size_of::<T>()` bytes by construction rather than by inspection. The half of the old contract the bound cannot carry is the document's half, that the stored element header is this type's own. That one is not a soundness matter -- taking a payload of the wrong width leaves the cursor inside the next value, which is a misparse -- so it stays a `# Correctness` note and a test each caller writes.

It is not sealed, and that is the deliberate part. The set of types whose memory is a BEVE payload is not the set of numbers this crate can name: it also holds every `#[repr(transparent)]` newtype over one that somebody else declared, which is exactly the type an adapter exists to describe. Sealing would have made the format's headline feature unreachable from the mechanism the orphan rule makes necessary. What keeps that safe is that the trait was already `unsafe` for the crate's own impls, so opening it adds an implementor rather than an obligation, and the one clause a compiler can check is checked for outside impls by the same arithmetic as inside ones, at the point of use rather than at the declaration: `numeric_bytes!` asserts it beside each impl in this file, where `cargo check` sees it, and `Block::<T>::WIDTH` asserts it wherever an impl from anywhere reaches a block, which is a full build.
