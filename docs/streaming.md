...
Every form whose payload is a block is accepted: a typed numeric array plain or aligned, and a **complex** array plain or aligned, whose interleaved `(re, im)` components are the in-memory form of `[Complex<T>]` for the same reason a typed array's payload is the in-memory form of `[T]`. A buffer of IQ samples is where this is worth the most, being the thing a consumer can least afford to hold twice.
